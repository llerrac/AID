...
uses
  ...
  IBCustomDataSet, IBDatabase, IBQuery, crIBQry,
  aiddbLkpPart, Buttons, crDlgLst, crdbEdit, crdbEd, aiddbLkpComp, crInfo,
  crdbInfo, IBSQL, ComCtrls, crPage, crdbLkp,
...
type
  TFrmFndPart = class(TcrDlgForm)
    ...
    GrdParam: TcrdbGrid;
    TabLevel: TTabSheet;
    QryParam: TcrIBDataset;
    QryParamPARTPARAM: TIBStringField;
    ...
    QryLookups: TIBQuery;
    QryLookupsKEY_LK: TIBStringField;
    QryLookupsDESC_LK: TIBStringField;
    MemLkpPartType: TkbmMemTable;
    SrcLkpPartType: TDataSource;
    MemLkpPartTypeKEY_LK: TIBStringField;
    MemLkpPartTypeDESC_LK: TIBStringField;
    LblBusDiv: TcrLabel;
    ...
    QryPartLASTTRANNO: TIBStringField;
    QryPartNEXTEXPECTED: TDateTimeField;
    QryPartNEXTTRANNO: TIBStringField;
...
function Execute(ADatabase: TIBDatabase; var APartDef: TvalPartDef; AOptions: TvalPartLkpOptions): Boolean;
...
begin
  ...
  //
  QryLookups.Database:= XuDatabase;
  LoadLookups;
  //
  MemSrch.Active:= True;
  MemSrch.Append;
  ...
  QryPart   .Database:= XuDatabase;
  QryPartEx .Database:= XuDatabase;
  QryParam  .Database:= XuDatabase;
...
//    QryPart.AddWhereClause(QryPartSuppNo    , MemSrchSuppNo    .AsString, cropLike , 'Supplier #: %s'   , crcoAND, False, 'pm.' );
    ...
    case TcrboParts(StrToInt(MemSrchOrderBy.Value)) of
      ...
      boBySuppNo:
      begin
        QryPart.AddOrderClause('ORDER BY pm.oemno2, pm.PartNoSeq');
        Grd.SortDisplay.SetSortField(QryPartOEMNO2)
      end;
      boByAka1:
      begin
...
begin
  ...
  QryPart.SQL.Add( '  pm.PartType,pm.SupplyDivision');//, pm.SupplyCode, ' );
...
begin
  aiddbUtil.GetCompanyInfo(QryPart.Database, Sender.AsString, rCompDef);
...
begin
  ...
  QryPartoemno2        .Visible:= not (valloAdministrative in FrOptions);
...
begin
  if ((Column.Field=QryPartPARTDESC) or (Column.Field=QryPartPARTNO) or (Column.Field=QryPartoemno2)) and
