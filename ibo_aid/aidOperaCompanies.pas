...
var
  ...
  uCO_PlNlCo: TField;
(*
  procedure Loadvia_Apollo;
  var
    tbl_SeqCo : TApolloTable;
  begin
    ...
    tbl_SeqCo:= TApolloTable.Create(nil);
    try
      tbl_SeqCo.DatabaseName:= sSystem;
      tbl_SeqCo.TableName   := 'SEQCO.DBF';
      tbl_SeqCo.Open;
      try
        while not tbl_SeqCo.Eof do
        begin
          {
          mem_SeqCo.Append;
          crdbUtil.CopyFieldsFromTo(tbl_SeqCo, mem_SeqCo);
          mem_SeqCoDataPath.AsString:= MangleFolderName(sSystem, mem_SeqCoCO_SUBDIR.AsString);
          mem_SeqCo.Post;
          }
          uCO_Code  := tbl_SeqCo.FindField('CO_CODE'  );
          uCO_Name  := tbl_SeqCo.FindField('CO_NAME'  );
          uCO_SubDir:= tbl_SeqCo.FindField('CO_SUBDIR');
          uCO_SlNlCo:= tbl_SeqCo.FindField('CO_SLNLCO');
          uCO_PlNlCo:= tbl_SeqCo.FindField('CO_PLNLCO');

          FillChar(rCompany, SizeOf(ToperaCompany), #0);

          if Assigned(uCO_Code  ) then rCompany.Co_Code  := StringToChar(uCO_Code  .AsString);
          if Assigned(uCO_Name  ) then rCompany.Co_Name  := uCO_Name  .AsString;
          if Assigned(uCO_SubDir) then rCompany.Co_SubDir:= uCO_SubDir.AsString;
          if Assigned(uCO_SlNlCo) then rCompany.Co_SlNlCo:= StringToChar(uCO_SlNlCo.AsString);
          if Assigned(uCO_PlNlCo) then rCompany.Co_PlNlCo:= StringToChar(uCO_PlNlCo.AsString);
          rCompany.DataPath:= MangleFolderName(sSystem, rCompany.Co_SubDir);
          ...
          if not IsEmptyStr(rCompany.Co_Code) then
          begin
            rCompany.Populated := True;
            AddCompany(rCompany.Co_Code, rCompany);
            AddToLog('Read: '+rCompany.Co_Code+'= '+rCompany.co_SubDir+' '+Bracket(rCompany.co_Name));
          end;
          tbl_SeqCo.Next;
        end;
      finally
...
begin
  ...
  case StringToChar(acSite) of
    ...
    'B',
    'C',
    'T' : Result:= 'B'; //  Bedford (Atlas Converting Limited    ( 2007 Consolidation of A & T )
