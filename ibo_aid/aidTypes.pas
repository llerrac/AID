...
type
  TApplicationAudience = (aidauBobst, aidauApolloSheeters);
...
type  { see BomTypes }
  ...
  TSectNo      = String [ 5];
  TBOMNo       = String [10];
  TPartNo      = String [10]; // VarChar(10)
  TPartParam   = String [10]; // VarChar(10)

  TJobNo          = String [10]; // VarChar(10)

  TSiteNo         = String [ 1]; // Char(1)
  TSiteCollection = String[26]; // VarChar(26) - A string containing an undelimited list of site IDs.

  TWarehouseNo    = String[4];
  TRowNo          = String[2];
  TBinNo          = String[3];

  TCompNo      = String [10]; // VarChar(10)
  TDesc50      = String [50]; // VarChar(50)
  TName        = String [50];
  TDesc20      = String [20];
  TModel       = String [10];
  TWeeks       = Integer;
  ...
  TAnalysisCoding = String[ 1];
  TAnalysisCode   = String[10];
  

type
  EaidException       = class(Exception);
...
const // Atlas Specific
  ...
  CUserPermissionClass          : Array[TUserPermissionClass] of string
    = ({ 1- 5} 'Company'      , 'Purchase'    , 'Part'       , 'Assembly'    , 'Stock',
       ...
       {21-25} 'Contract Info', 'Wire-Less'   , 'User Report', '** Spare **', '** Spare **' ,
...
type { States per Contract }
  TaidContState = ( aidCSDesign, aidCSProcure, aidCSBuild, aidCSShipped, aidCSCommissioning, aidCSWarranty, aidCSSpares, aidCSCancelled,aidCSInstallation );
...
type
  TaidDocumentType    = (aidDocTypeUnknown, aidDocTypeDrawing, aidDocTypeScan, aidDocTypeInfo, aidDocTypeDocument, aidDocTypeManual, aidDocTypeCertificate, aidDocTypeLink);
...
Type
  ...
  TaidCheckoutState = (aidUnlocked, aidCheckedOut, aidUpIssuePending, aidDeletionPending, aidCOAddition);
  ...
  EaidVCSException  = class(Exception);
...
type
  TBinType = (binStock, binASA, binConsumable, binConvert, binContract, binReserved, binTemporary);
...
Const
  CaidContState       : Array[TaidContState ] of char   = ( 'D', 'P', 'B', 'I', 'M', 'W', 'S', 'X','N' );
  CaidContStateDesc   : Array[TaidContState ] of string = ( rsCSDesign, rsCSProcure, rsCSBuild, rsCSShipped, rsCSCommissioning, rsCSWarranty, rsCSSpares, rsCSCancelled, rsCSINstallation );
  CaidContStateHint   : Array[TaidContState ] of string = ( rsCSHintDesign, rsCSHintProcure, rsCSHintBuild, rsCSHintShipped, rsCSHintCommissioning, rsCSHintWarranty, rsCSHintSpares, rsCSHintCancelled ,rsCSHintInstallation);
  CaidContStateBGColor: Array[TaidContState ] of TColor = ( clWhite, clYellow, clSkyBlue, clRed  , clLime , clTeal , clMoneyGreen, clOlive, clGreen );
  CaidContStateFGColor: Array[TaidContState ] of TColor = ( clBlack, clBlack , clBlack  , clWhite, clBlack, clBlack, clBlack     , clBlack ,clBlack );
begin
...
function  CheckoutStateRecord  (AeState: TaidCheckoutState): TaidCheckoutStateRecord;
const
  CaidCheckoutStateKey : Array[TaidCheckoutState] of Char   = ('U', 'O', 'P', 'D','N');
  CaidCheckoutStateHint: Array[TaidCheckoutState] of String = ('Checked in', 'Checked out', 'Pending up-issue', 'Pending Deletion','New Item');
  CaidCheckoutColorBG  : Array[TaidCheckoutState] of TColor = (clWindow    , clBlack, clYellow, clRed  , aidCl_NewItem );
  CaidCheckoutColorFG  : Array[TaidCheckoutState] of TColor = (clWindowText, clWhite, clBlack , clWhite, clBlack );
begin
...
function BinTypeRecordByOrd(AeBinType: TBinType): TBinTypeRecord;
const
  CBinTypeKey   : Array [TBinType] of string = ('S', 'A', 'O', 'V', 'C', 'R', 'T');
  CBinTypeDesc  : Array [TBinType] of string = ('Stock', 'ASA', 'cOnsumable', 'conVerting', 'Contract', 'Reserved', 'Temporary');
  CBinTypeBG    : Array [TBinType] of TColor =
    (clYellow, clPurple, clPurple, clMaroon, clOlive, clTeal , clCream);
  CBinTypeFG    : Array [TBinType] of TColor =
    (clBlack , clWhite,  clYellow, clWhite,  clWhite, clWhite, clBlack);
begin
  FillChar(Result, SizeOf(Result), #0);
  Result.Key    := CBinTypeKey [AeBinType];
  Result.Desc   := CBinTypeDesc[AeBinType];
