...
type
  TaiddbLkpSiteFilterStyle =
    (alsiAll, alsiOnlyActive, alsiActiveOrFieldObsolete, alsiCustom,alsiActiveOrReference);
type
...
type
  ...
  public
    Constructor Create(AsSiteNo : TSiteNo; AsSiteDesc, AsRegion: String; AbIsObsolete: Boolean;aIsReference : boolean);
...
{ TaiddbLkpSiteObject }
constructor TaiddbLkpSiteObject.Create(AsSiteNo: TSiteNo; AsSiteDesc, AsRegion: String; AbIsObsolete: Boolean;aIsReference : boolean);
begin
  FsSiteNo      := AsSiteNo;
...
begin
  ...
  if IsReference then
  begin
    AoCanvas.Pen.Color:= clYellow;
    AoCanvas.Pen.Style := psDash;
    AoCanvas.MoveTo (rDescRect.Left , iMid-1);
    AoCanvas.LineTo (rDescRect.Right, iMid-1);
  end;
end;
