...
uses
  classes, contnrs,
  aidTypes, aidCommsObj, aidCommsTypes, IB_Session, IBODataset, IB_Components;
...
type
  ...
  public
    ...
    function EmailAsXML_ansistring: Ansistring;
  published
...
type
  ...
  private
    ...
    procedure AddEmailTodbQueue(AConfig: TaidCommunicationArg;AEmailItem: TaidCommunicationEmail; AsXML: ansistring);
...
uses
  SysUtils, StrUtils, db, SZCodeBaseX ,crUtil,aidDatbaseLifetime, aid_database_ibo,u_tBOMKernel;

function TaidCommunicationEmailObj.EmailAsXML_ansistring: Ansistring;
begin
  //implicit conversion!
  result := EmailAsXML
end;
...
begin
  ...
  case Email.MessageFormat of
    ...
    aidEmailHTML     : result:= result+  '<body message="'+ SZCodeBaseX .SZEncodeBase64(email.MailMessage)+'" > </body>'#13#10;
  end;
          ...
          begin
            ...
            begin
              AddEmailTodbQueue(
                ...
                TaidCommunicationEmailObj(FQueue.Items[iPos]).EmailAsXML_ansistring);
            end;
          end;
          inc(iPos);
        end;
...
procedure TaidCommunicationsThread.AddEmailTodbQueue(AConfig: TaidCommunicationArg;AEmailItem: TaidCommunicationEmail; AsXML: ansistring);
var
  iInc          : Integer;
  iK            : Integer;
  oStr          : TStringStream;
...
begin
  ...
  begin
    aid_database_ibo.LookUpDbTrans(FCommsData.CommsDbAlias,IB_ConnectionComms , '');
    if not IB_ConnectionComms.connected then
    //if not
    //AssignDatabaseDetails(IB_ConnectionComms , FCommsData.CommsDbAlias) then
    begin
