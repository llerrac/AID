...
uses
  ...
  IBCustomDataSet, IBDatabase, IBStoredProc, 
  { 3rd Party Units }
  { Application Units }
...
type
  TFrmMtnLookupDropins = class(TcrDlgForm)
    ...
    qryNteGROUP_LK: TIBStringField;
    qryNteDESC_LK: TIBStringField;
    srcNte: TDataSource;
    genInc: TIBStoredProc;
    GrpDesc: TcrGroupBox;
    LblDesc: TLabel;
    EdtDesc: TDBEdit;
    ...
    qryNteCREATEDBY: TIBStringField;
    qryNteALTEREDDTE: TDateTimeField;
    qryNteALTEREDBY: TIBStringField;
    pnlBtns: TPanel;
    act: TActionList;
    img: TImageList;
...
var
  ...
  function Execute(AoDatabase: TIBDatabase; AsGroup: string; AsCaption: String = 'Maintain Notes'; AsUserNo: String = ''): Boolean;
...
begin
  ...
  //
  ibTran.Active:= False;
  ibTran.DefaultDatabase:=  Xdb;
  QryNte.Database:=  Xdb;
  ibTran.StartTransaction;
  //
  QryNte.Active:= False;
  QryNte.Params.ByName('Group_Lk').AsString:= FsGroup;
  QryNte.Active:= True;
end;
