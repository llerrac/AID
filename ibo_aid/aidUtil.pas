...
function  FormatCommodityCode(AsText: String): String;
...
begin
  ...
  if AsReqnState = 'P' then iReqnColor:= clLime else
    if AsReqnState = 'F' then iReqnColor:= clGreen else
      if AsReqnState = '-' then iReqnColor:= clGray else
  ...
  if AsActdState = 'P' then iActdColor:= clYellow else
    if AsActdState = 'F' then iActdColor:= clOlive else
      if AsActdState = '-' then iActdColor:= clGray else
  ...
  if AsRcvdState = 'P' then iRcvdColor:= clRed else
    if AsRcvdState = 'F' then iRcvdColor:= clMaroon else
      if AsRcvdState = '-' then iRcvdColor:= clGray else
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if ((cLeft  in CSeps) or ((AlLeadingDigit) and (cLeft  in ['0'..'9']))) and
           ((cRight in CSEPS) or ((AlLeadingDigit) and (cRight in ['0'..'9']))) then
        begin
...
begin
  ...
  begin
    while (iX <= Length(sArg)) and (sArg[iX] in CSeps) do
      Inc(iX);
